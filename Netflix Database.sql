...
#1. Using the Viewing History table, identify the top 3 most-watched movies based onviewing hours.
select v.runtime , c.titlename
 from viewinghistory v
 join content c on v.contentid=c.contentid
 group by c.titlename , v.runtime
 order by v.runtime desc
 limit 3;

#2. Partition the viewing hours by category and genre to find the top genre in eachcategory. 
#Use the rank function to rank genres within each category.

select sum(v.runtime ) as viewing_hours,
c.category , c.genre, 
...
#Plan Name andSubscriber count in descending order of Subscriber count.

select count(s.custid) as subscribers_count, p.planid , p.planName
from subscribes s
join plans p on s.planid=p.planid
group by p.planid , p.planName
order by subscribers_count desc  ;

#4. Which device type is most commonly used to access Netflix content? Provide the DeviceType and count of accesses.
select d.devicetype , count(*) as access_count
from uses u
join devices d on u.deviceid=d.deviceid
group by d.devicetype
order by access_count desc 
limit 1;

#5. Compare the viewing trends of movies versus TV shows. What is the average viewingtime for movies and TV shows separately?
select avg(v.runtime) as average_viewtime , c.category
from content c
join viewinghistory v on c.contentid=v.contentid
group by c.category
order by average_viewtime desc; 

#  6.Identify the most preferred language by customers. Provide the number of customers,and language.
select count(cu.custid) as no_of_customers , cu.language
from customerslanguagepreferred cu
join customers c on cu.custid=c.custid
group by cu.language
...
SELECT 'Child Account' AS AccountType, 
COUNT(DISTINCT p.CustID) AS CustomerCount
FROM childacc c
JOIN profiles p ON c.ProfileID = p.ProfileID
UNION
SELECT 'Adult Account' AS AccountType, 
COUNT(DISTINCT p.CustID) AS CustomerCount
FROM adultacc a
JOIN profiles p ON a.ProfileID = p.ProfileID;

#8. Determine the average number of profiles created per customer account.
select  count(p.profileid)  / count(distinct c.custid) as average_profile
from profiles p
...
select round(avg(v.runtime),2) as average_viewtime , c.titlename , v.profileid
from viewinghistory v
join content c on v.contentid=c.contentid
group by c.titlename , v.profileid
order by average_viewtime asc
limit 5 ;

#10. Determine the count for each content type.
select category , count(*) as content_type
from content
group by category;

#11 Compare the number of customers that have unlimited access and who do not.
select c.unlimitedaccess , count(distinct p.custid) as no_of_customers
from content c
join viewinghistory v on c.contentid=v.contentid
join profiles p on v.profileid=p.profileid
group by c.unlimitedaccess;

#12. Find Average monthly price for plans with Content Access as "unlimited".
select contentaccess , avg(monthlyprice) as average_monthly_price
from plans
...
#13. List all the customers who have taken the plan for till 2028 and later.
 #Display CustomerID, Customer name and Expiration Date of the plan, ordered by Expiration
 #Date in descending order first, and then by Customer Name.
 select c.custid, concat(fname," ",lname) as customer_name, expirationdate
 from customers c 
 join paymentmethod p on c.custid = p.custid
 where expirationdate>='2028-01-01'
 group by c.custid , expirationdate  
 order by expirationdate asc , customer_name ;
 
#14. Display Average Revenue generated from each city. Rank city based on average revenue.
select avg(pa.paymentamount) as average_revenue, p.city,
rank() over (order by avg(pa.paymentamount) desc ) as revenue_rank
 from paymentmethod p
 join paymenthistory pa on p.cardID =pa.cardID
 group by p.city;

#15. Display most frequently viewed genre among adults for each category.
select c.category,c.genre,v.runtime , count(*) as view_count
from content c
join viewinghistory v on c.contentid=v.contentid
join profiles p on v.profileid=.profileid
group by c.category , c.genre
order by v.runtime desc; 
